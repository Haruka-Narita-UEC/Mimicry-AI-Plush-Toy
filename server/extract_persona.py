...
PROMPT_PATH = "prompts/prompt_extractpersona.txt"
MODEL = "gemma3:12b-it-qat"

def generate_persona(conversation_txt: str):
    try:
...
def save_dict_yaml(yaml_text: str, raw_response: str):
    ...
    if isinstance(data_dict, dict):
        filename = f"extract_yaml/extract_persona_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.yaml"
...
def main():
    parser = argparse.ArgumentParser(description="Extract Persona from JSONL")
    ...
    response = generate_persona(conversation_log)
